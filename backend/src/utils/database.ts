...
type ExcludeDefaultField<T> = Omit<T, 'id' | 'created_at'>;
...
const getWhereStatement = <Data extends Record<string, any>>(
  ...
  _relation?: {
    ...
    orderBy?: 'ASC' | 'DESC';
    sortBy?: keyof Data | { specialCase: string };
    limit?: number;
    offset?: number;
  }
) => {
  ...
  let sorting = sortBy
    ? typeof sortBy === 'object' && sortBy?.specialCase
      ? `ORDER BY ${sortBy.specialCase}`
      : ` ORDER BY ${String(sortBy)} ${orderBy ?? 'ASC'}`
    : ` ORDER BY [${mainTableName}].created_at ASC`;
...
export const getSelectQuery = <T1 extends TableName, N1 extends string = T1>(
  ...
  function getMethods<Data extends Record<string, any>>(
    ...
    return {
      ...
      selectGroupBy: selectGroupBy<Data>(payload),
    };
  }
  ...
  function mainTableSelect<T2 extends Record<string, any>>(
    ...
    return function <T3 extends Record<string, keyof MainDatabase[T1]> | '*'>(
      ...
      _relation?: {
        relation?: ConditionRelation;
        sortBy?: keyof T2 | { specialCase: string };
        orderBy?: 'ASC' | 'DESC';
        limit?: number;
        offset?: number;
      }
  ...
  function selectGroup<T1 extends Record<string, any>>(payload: SelectPayload) {
    ...
    >(
      ...
      const whereStatement = getWhereStatement(mainTableName, options, {
        relation: relation,
      });
  ...
  function selectGroupBy<T1 extends Record<string, any>>(
    payload: SelectPayload
  ) {
  ...
  `;
      ...
      const runQuery = createRunQuery(async query => {
        ...
        const gb = groupByField as string;

        return result.flatMap(a => a[gb.replace(`${mainTableName}.`, '')]);
      });
...
export const getInsertQuery = <T extends TableName>(
  tableName: T,
  payload:
    | ExcludeDefaultField<MainDatabase[T]>
    | ExcludeDefaultField<MainDatabase[T]>[]
) => {
...
export const getUpdateQuery = <T extends TableName>(
  tableName: T,
  payload: GetUpdateData<ExcludeDefaultField<MainDatabase[T]>>,
  conditions?: Partial<SelectDatabaseValue<MainDatabase[T]>>
) => {
...
export const getDeleteQuery = <T extends TableName>(
  tableName: T,
  conditions?:
    | Partial<SelectDatabaseValue<MainDatabase[T]>>
    | Partial<SelectDatabaseValue<MainDatabase[T]>>[],
